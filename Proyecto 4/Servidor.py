...
lock = threading.Lock() # Bloqueo para manejar operaicon en estructuras compartidas
            ...
            if modo == "1":  # Modo de inicio de sesión
                    ...
                    if usuario in usuarios and bcrypt.verify(contraseña, usuarios[usuario]):
                        conexion.send(
                            ...
                            "1. Agregar amigo\n"
                            "2. Ver amigos\n"
                            "3. Ver quiénes te siguen".encode('utf-8')
                        )
                        ...
                        while True:
                            ...
                            if comando.lower() == "salir":
                                ...
                                conexion.send(f"Tus amigos: {', '.join(amigos) if amigos else 'No tienes amigos :('}".encode('utf-8'))

                            elif comando == "3":  # Ver quiénes te siguen
                                with lock:
                                    seguidores = amistades.obtener_amigos(usuario)
                                conexion.send(f"Usuarios que te siguen: {', '.join(seguidores) if seguidores else 'Nadie te sigue aún.'}".encode('utf-8'))
...
try:
    while True:
        ...
        # Crear u hilo par menejar al cliente
